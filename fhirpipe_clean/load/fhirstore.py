...
def get_mongo_client():
    ...
    if _client is None:
        _client = MongoClient(
            host=fhirpipe_clean.global_config.fhirstore.host,
            port=fhirpipe_clean.global_config.fhirstore.port,
            username=fhirpipe_clean.global_config.fhirstore.user,
            password=fhirpipe_clean.global_config.fhirstore.password,
        )
    return _client
...
def get_fhirstore():
    ...
    if _fhirstore is None:
        _fhirstore = fhirstore.FHIRStore(
            get_mongo_client(), fhirpipe_clean.global_config.fhirstore.database
        )
    return _fhirstore
...
def find_fhir_resource(resource_type, identifier):
    ...
    """
    db_client = get_mongo_client()[fhirpipe_clean.global_config.fhirstore.database]
